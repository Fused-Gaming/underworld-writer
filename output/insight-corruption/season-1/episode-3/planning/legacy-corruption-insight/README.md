# Episode 3: The Building Inspector's Decade of Bribes

![Insight Corruption](../../../../production/assets/linkedin-headers/ci-3-broadsheet.png)

**Subtitle:** Rodolfo Pada's Bribery Syndicate

| Field | Detail |
| --- | --- |
| Location | San Francisco, California |
| Year | 2026 (recent sentencing) |
| Offense | Accepted $40,000+ in bribes over 14 years (2003-2017) from construction firms |
| Activities | Approved fraudulent permits, expedited inspections, approved unsafe building plans |
| Sentence | 1 year + 1 day |
| Escape Mechanism | 14 years of corruption treated as a single offense with minimal sentence |
| Case File | `../../characters/rodolfo-pada-case.json` |

## Investigative Angle

How did 14 years of systematic bribery get treated so leniently? This episode unpacks how a long-running scheme got bundled into a single, minor sentencing event.

## Status

`ready-for-script-generation`
