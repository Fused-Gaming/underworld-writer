...
const templatePath = path.join(root, 'assets', 'branding', 'underworld-writer-release-template-1200x630.svg');
...
function parseArtworkItems(release) {
  ...
  const fallback = ['Added', 'Changed', 'Fixed']
...
function cardText(item, x) {
  const desc = wrap(item.description, 27, 2);
  return `
    <g data-release-art-card="true">
      <rect x="${x - 7}" y="447" width="166" height="80" rx="5" fill="#070B14" fill-opacity="0.98"/>
...
const defaults = [
  ...
  { title: 'REAL IMPACT', description: 'Fiction, true-crime, investigative stories' },
  { title: 'BUILT FOR MORE', description: 'Episodes. Series. A transparent tomorrow.' },
];
...
const dynamicLayer = `
    ...
    <g id="underworld-release-dynamic" pointer-events="none">
      <rect x="52" y="281" width="156" height="43" rx="8" fill="#0A0710" fill-opacity="0.98"/>
...
if (checkOnly) {
  ...
  if (existing !== rendered) {
    throw new Error('Release artwork is stale. Run: npm run release:art');
  }
